...
module Iu::Ui
  class Window < Iu::Widget::Control
    ...
    def child=(child : Iu::Widget::Widget)
      child.parent = self
      UI.window_set_child(to_unsafe, ui_control(child.control.as(Iu::Widget::Control).to_unsafe))
    end
    ...
    def on_close=(proc : Proc(Window, Nil))
      boxed_data = ::Box.box(proc)
